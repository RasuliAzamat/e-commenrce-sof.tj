    ...
    <div class="flex flex-col">
      ...
      <div class="flex justify-between">
        <div class="text-xl">
          ...
          <small class="text-yellow-800"> {{ toCorrectCurrency(product.price) }} смн </small>
        </div>

        <button
          :class="['p-2 rounded-full self-end', cartIconColor]"
          @click.stop.prevent="addToCart(product.id), $router.push('/cart')"
        >
          <img src="/img/icons/product-cart.svg" alt="Иконка корзины" />
        </button>
      </div>
    </div>
  </NuxtLink>
