<template>
    <section class="container max-w-6xl px-4 ml-auto mr-8">

        <div class="flex items-center justify-between mb-12">
            <h2 class="text-danger">Последние добавления</h2>
            <NuxtLink to="/products" class="text-2xl no-underline">
                Cмотреть все
            </NuxtLink>
        </div>
        ...
        >
            <swiper-slide v-for="product in lastAddedProducts" :key="product.id" >
                <GlobalProductItem :product="product" class="w-full h-full" />
            </swiper-slide>
        </swiper>
